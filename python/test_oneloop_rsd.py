...
pkmu_rsd_spline = cosmo.eft_pkmu_rsd_spline(mu_of_z, z, bias, counter, 'Pdd_hh_rsd')
...
pkmu_rsd_spline_grid = cosmo.eft_pkmu_rsd_grid(mu_of_z, k_spl_of_z_mu, z, bias, counter, 'Pdd_hh_rsd')

#print(pkmu_rsd_spline_grid[id_z, 0, 0])
#print(pkmu_rsd_spline.c[:, 0, id_z, 0])

plt.figure(1)
plt.loglog(k_grid, (bias[id_z, 0]**2.)*pk_lin[id_z], label='$b_1^2 \cdot$linear')
...
#plt.ylim(10., 50.e3)
plt.legend()
plt.savefig('plots/oneloop_rsd_spectra.pdf')