...
int fourier_init(
  ...
  /** --> Then go through common preliminary steps to the HALOFIT and HMcode methods */
    ...
    //              pfo->error_message);
    double pk_nl;
    if (pfo->k[pfo->index_pk_m] <= 0.01) {
      fourier_pk_at_k_and_z(pba, ppm, pfo, pk_linear, pfo->k[pfo->index_pk_m], z, pfo -> index_pk_cb, &pk_nl, NULL); 
      pfo->ln_pk_nl[index_pk][(pfo->ln_tau_size-1)*pfo->k_size+pfo->index_pk_m] = log(pk_nl);
      }
    else{
      class_call(PS_hh_G(ppr, pba, ppt, ppm, pfo, index_pk, pfo->index_pk_m, z, 0.0, 145L, 147L, 141L, 0.0),
                 pfo->error_message,
                 pfo->error_message);
    }
  }
