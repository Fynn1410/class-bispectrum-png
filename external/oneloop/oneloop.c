 ...
 * edited by Julien Lesgourgues, Nov 2024
 *
...
int oneloop_linear_spectrum_rsd(
  ...
  if (arg_type == cartesian_product) {
    ...
    vec_size = kvec_size; /** = muvec_size */
    class_call(eft_rsd_argument_list(ln_kvec,
                                     muvec,
                                     vec_size,
                                     &vec,
                                     peft->error_message),
 ...
 * @brief TBF
 *
 * @param peft       Input/Output: TBF
 * @param pext       Input/Output: TBF
 * @param index      Input: TBF
 *
 * @return the error status
 */

int oneloop_get_loop_matrices(
                              struct eft * peft,
  ...
  case eft_master:
    ...
    if (ext_insert_eft(pext, peft, index, peft->moments_allocated, errmsg) != _SUCCESS_) {
      ...
      class_call(oneloop_allocate_loop_matrices(peft), peft->error_message, peft->error_message);
      ...
      if (peft->hp->compute_loop_matrices) {  /** - compute the loop matrices or */
        ...
        if (peft->hp->write_loop_matrices == _TRUE_) {  /** - and save them if flag is set */
          for (index_moment = 0; index_moment < peft->moments_allocated; index_moment++) {
            ...
            class_call(oneloop_save_matrix_to_file(peft->loop_matrices[index_moment],
        ...
        for (index_moment = 0; index_moment < peft->moments_allocated; index_moment++) {
          if (peft->loop_matrices_size[index_moment] == 0) { continue; }
          class_sprintf(filename, "%s_%03d.mat", peft->hp->eft_loop_matrix_files[index_moment], index);
          // class_call(oneloop_read_matrix_from_file(peft->loop_matrices[index_moment],
          //                                               peft->loop_matrices_size[index_moment],
          //                                               peft->symmetry[index_moment],
          //                                               peft->use_tracer[index_moment],
          //                                               peft->hp->period[peft->use_tracer[index_moment]],
          ...
          if (oneloop_read_matrix_from_file(peft->loop_matrices[index_moment],
                                            peft->loop_matrices_size[index_moment],
                                            peft->symmetry[index_moment],
                                            peft->use_tracer[index_moment],
                                            peft->hp->period[peft->use_tracer[index_moment]],
...
int oneloop_nearest_structure_in_time(
  ...
  *index_eft_min_dist = 0;
  min_distance = fabs(z - pfo->z_pk_eft[0]);

  /** - find nearest eft structure */
  for (index_eft = 1; index_eft < pfo->z_pk_eft_num; index_eft++) {
    distance = fabs(z - pfo->z_pk_eft[index_eft]);
    if (distance < min_distance) {
      min_distance = distance;
      *index_eft_min_dist = index_eft;
    }
  }
...
// define a utility function with type int, used as an argument of qsort()
int indexed_double_cmp_inc(
                           const void * a,
                           const void * b
                           ) {
...
int oneloop_job_powerspectrum_wedges_ext_growth_rate(
                                                     struct eft * peft0,
                                                     int peft_size,
                                                     double * f_z_pk_eft,
                                                     double * D_z_pk_eft,
  ...
  int index_pk_type, index_spectra_contribution, index_list;
  ...
  for (index_z_sort = 0; index_z_sort < z_size; index_z_sort++) {
    ...
    class_call(oneloop_nearest_structure_in_time(peft0, peft_size, pba, pfo,
                                                 z,
  ...
  for (index_z_sort = 0; index_z_sort < z_size; index_z_sort++) {  /** - main z-loop */
    ...
    if (list_pk_types_loops_not_loaded_size > 0 && peft->hp->integration_mode == fftlog) {
      ...
      class_call(oneloop_fourier_transform_linear_spectra(ppr, pba, pfo, ppm,
                                                          peft,
                                                          z,
                                                          f_z,
                                                          D_z,
...
//  * @return the error status
//  */
// int oneloop_job_powerspectrum_wedges_averaged_ext_growth_rate(
//                                 struct eft * peft0,
//                                 int peft_size,
//                                 double * f_z_pk_eft,
//                                 double * D_z_pk_eft,
...
//   /** - apply the AP effect */
//   oneloop_apply_ap_effect_in_place(kvec_true,
//                                k_sizevec,
//                                muvec_true,
//                                mu_sizevec,
//                                z_size,
...
//   class_call(oneloop_job_powerspectrum_wedges_ext_growth_rate(peft0,
//                                                           peft_size,
//                                                           f_z_pk_eft,
//                                                           D_z_pk_eft,
//                                                           pba,
...
int oneloop_job_powerspectrum_wedges_grid(
  ...
  class_call(oneloop_job_powerspectrum_wedges(peft0,
                                              peft_size,
                                              pba,
                                              pfo,
                                              ppm,
...
int oneloop_job_powerspectrum_multipoles_ext_growth_rate(
                                                         struct eft * peft0,
                                                         int peft_size,
                                                         double * f_z_pk_eft,
                                                         double * D_z_pk_eft,
  ...
  int index_z, index_l, index_k, index_mu;
  ...
  for (index_z = 0; index_z < z_size; index_z++) {
    mu_sizevec[index_z] = GL_RULE_MULIPOLE_SYM_SIZE;
    ...
    memcpy(muvec_true[index_z], gl_multipole_sym_abscissa, mu_sizevec[index_z]*sizeof(double));
  }

  /** - apply the AP effect */
  class_call(oneloop_apply_ap_effect_in_place(kvec_true,
                                              k_sizevec,
                                              muvec_true,
                                              mu_sizevec,
                                              z_size,
  ...
  for (index_z = 0; index_z < z_size; index_z++) {
    for (index_l = 0; index_l < MULTIPOLE_SIZE; index_l++) {
      for (index_k = 0; index_k < k_sizevec[index_z]; index_k++) {
        ...
        double out_pkl_p;
        out_pkl_p = 0.;
        for (index_mu_p = 0; index_mu_p < mu_sizevec[index_z]; index_mu_p++) {
          out_pkl_p += gl_multipole_sym_weights[index_mu_p] * lg_measure[index_mu_p][index_l] \
            * out_pkmu[index_z][index_mu_p*k_sizevec[index_z] + index_k] \
            / (ap_parallel[index_z] * ap_perpendicular[index_z] * ap_perpendicular[index_z]);
        }
        out_pkl[index_z][index_l*k_sizevec[index_z] + index_k] = 0.5*(4*index_l + 1)*out_pkl_p;
        //return _SUCCESS_;
        //);
      }
    }
...
int oneloop_job_powerspectrum_multipoles(
  ...
  */
  ...
  fprintf(stderr,"In eft_job_powerspectrum_multipoles, time = %.6f s\n",
          elapsed
          );
  */
...
int oneloop_init(
  ...
  class_call(oneloop_indices(peft, eft_role), peft->error_message, peft->error_message);
  ...
  if (eft_role == eft_master) {
    for (index_tracer = 0; index_tracer < eft_tracer_num; index_tracer++) {
      ...
      class_call(oneloop_spline_sample_points_nonuniform(peft->hp->kmin_lin[index_tracer],
                                                         exp(ppr->k_bao_center),
                                                         peft->hp->kmax_lin[index_tracer],
                                                         peft->hp->bao_oversampling,
                                                         peft->hp->ln_k_oversampling_width,
      ...
      class_call(oneloop_get_fourier_frequencies(peft->hp->period[index_tracer],
                                                 peft->hp->fourier_coeff_size,
                                                 peft->fourier_frequencies[index_tracer]),
                 peft->error_message,
                 peft->error_message);
    }
    ...
    class_call(oneloop_spline_sample_points_nonuniform(peft->hp->k_IR_cutoff,
                                                      exp(ppr->k_bao_center),
                                                      peft->hp->k_UV_cutoff,
                                                      peft->hp->bao_oversampling,
                                                      peft->hp->ln_k_oversampling_width,
    ...
    if (peft->hp->use_interpolation) {
      class_call(oneloop_spline_sample_points_nonuniform(peft->hp->kmin_nl,
                                                         peft->hp->k_feature_nl,
                                                         peft->hp->kmax_nl,
                                                         peft->hp->bao_oversampling,
                                                         peft->hp->ln_k_oversampling_width_nl,
                                                         peft->hp->k_size_nl,
                                                         peft->ln_k, peft->hp->eft_verbose),
                 peft->error_message,
                 peft->error_message);
    }
...
int oneloop_spline_sample_points_nonuniform(
                                            double k_min,
                                            double k_feature,
                                            double k_max,
...
int oneloop_allocate_loop_matrices(
                                   struct eft * peft
                                   ) {
  int i;
...
int oneloop_get_fourier_frequencies(
                                    double period,
                                    int num_frequencies,
                                    double * frequencies
                                    ) {
...
int oneloop_fourier_transform_linear_spectra(
                                             struct precision * ppr,
                                             struct background * pba,
                                             struct fourier * pfo,
                                             struct primordial * ppm,
  ...
  if (peft->hp->integration_mode == fftlog) {
    ...
    for (index_list = 0; index_list < index_pk_types_size; index_list++) {
      ...
      for (index_tracer = 0; index_tracer < eft_tracer_num; index_tracer++) {
        ...
        class_call(oneloop_linear_spectrum_real(pba, ppm, pfo, peft, linear,
                                                peft->ln_k_fourier[index_tracer],
                                                peft->hp->k_size_fourier-1,
                                                1,
                                                peft->z0,
    ...
    for (index_list = 0; index_list < index_pk_types_size; index_list++) {
      ...
      for (index_tracer = 0; index_tracer < eft_tracer_num; index_tracer++) {
        ...
        //=,
        int it_p;
        for (it_p = 0; it_p < peft->hp->k_size_fourier-1; it_p++) {
          /** Apply the bias to the power spectra */
          peft->pk_l_biased[index_pk_type*eft_tracer_num + index_tracer][it_p] \
            *= exp(-peft->hp->bias[index_tracer] * peft->ln_k_fourier[index_tracer][it_p]);
        }
    ...
    //     }
    //     class_call(oneloop_linear_spectrum_rsd(pba, ppm, pfo, peft, linear,
    //                                                 peft->ln_k_fourier[index_tracer],
    //                                                 peft->hp->k_size_fourier-1,
    //                                                 peft->mu,
    //                                                 peft->mu_size,
    ...
    for (index_list = 0; index_list < index_pk_types_size; index_list++) {
      ...
      for (index_mu = 0; index_mu < mu_size; index_mu++) {
        ...
        for (index_tracer_p = 0; index_tracer_p < eft_tracer_num; index_tracer_p++) {
          for (it_p = 0; it_p < peft->hp->fourier_coeff_size-1; it_p++) {
            /** - equidistant sample points; last point ln(k_min) + period is left out since it is known by periodicity  */
            ln_k_fft_p = log(peft->hp->kmin_lin[index_tracer_p]) + peft->hp->period[index_tracer_p] * it_p/(double)(peft->hp->fourier_coeff_size-1);
            class_call(array_interpolate_spline_growing_closeby(peft->ln_k_fourier[index_tracer_p],
                                                                peft->hp->k_size_fourier,
                                                                peft->pk_l_biased[index_pk_type*eft_tracer_num + index_tracer_p] + index_mu*peft->hp->k_size_fourier,
                                                                peft->ddpk_l_biased[index_pk_type*eft_tracer_num + index_tracer_p] + index_mu*peft->hp->k_size_fourier,
                                                                1,
                                                                ln_k_fft_p,
                                                                &last_index_p,
                                                                &(pk_l_biased_fft[index_tracer_p][it_p]),
                                                                1,
                                                                peft->error_message),
                       peft->error_message,
                       peft->error_message);
        ...
        if (eft_tracer_num == 2) {
          ...
          for (index_tracer_p = 0; index_tracer_p < 2; index_tracer_p++) {
            ...
            peft->fourier_coeff[index_pk_type*eft_tracer_num + index_tracer_p][index_mu*peft->hp->fourier_coeff_size + 0] \
              = class_complex(fourier_coeff_real[index_tracer_p][0], fourier_coeff_imag[index_tracer_p][0]) / (double)(peft->hp->fourier_coeff_size-1);
            /** - positive frequency components in standard order, last iteration extends the spectrum symmetrically */
            for (it_p = 1; it_p < num_independent_coefficients; it_p++) {
              peft->fourier_coeff[index_pk_type*eft_tracer_num + index_tracer_p][index_mu*peft->hp->fourier_coeff_size + it_p] \
                = class_complex(fourier_coeff_real[index_tracer_p][it_p], fourier_coeff_imag[index_tracer_p][it_p]) * cpow(peft->hp->kmin_lin[index_tracer_p], -_Complex_I*peft->fourier_frequencies[index_tracer_p][it_p]) / (double)(peft->hp->fourier_coeff_size-1);
            }
            /** - negative frequency components in standard order are fully dependent on the others */
            for (it_p = -(num_independent_coefficients-1); it_p < 0; it_p++) {
              peft->fourier_coeff[index_pk_type*eft_tracer_num + index_tracer_p][index_mu*peft->hp->fourier_coeff_size + peft->hp->fourier_coeff_size + it_p] \
                = conj( peft->fourier_coeff[index_pk_type*eft_tracer_num + index_tracer_p][index_mu*peft->hp->fourier_coeff_size - it_p] );
            }
          }
        }
        else {
...
int oneloop_save_matrix_to_file(
                                class_complex * matrix,
                                int size,
                                short symmetry,
                                short tracer,
                                double period,
...
int oneloop_read_matrix_from_file(
                                  class_complex * matrix,
                                  int size,
                                  short symmetry,
                                  short tracer,
...
int oneloop_apply_ap_effect_in_place(
                                     double ** kvec,
                                     int * k_sizevec,
                                     double ** muvec,
                                     int * mu_sizevec,
