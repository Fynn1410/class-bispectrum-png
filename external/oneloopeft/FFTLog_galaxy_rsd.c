...
double P22_new(struct fft_struct *fft_input, double k, double z, int cleanup)
{
      ...
      for (i=0; i<Nmax+1; i++){
            vec1[i] = fft_input->cmsym_m[i] * cpow(k,fft_input->etam_m[i]);
            for (j=i; j<Nmax+1; j++){                        
                  nu1 = -0.5 * fft_input->etam_m[i];
                  nu2 = -0.5 * fft_input->etam_m[j];
                  // printf("M22_mat %d %d %12.6e %12.6e %12.6e  \n", i, j, k,  creal(M22_mat(nu1,nu2)), cimag(M22_mat(nu1,nu2)));
                  
                  M22_mat[i][j] = M22(nu1,nu2);
                  M22_mat[j][i] = M22(nu1,nu2);
            }
      }
...
void rsd_0_FFTLog(struct fft_struct  *fft_input, double k, double z, int cleanup, double *pg_loops)
{
      // int Nmax     = fft_input -> nfft;
